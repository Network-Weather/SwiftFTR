...
struct SwiftFTRCommand: AsyncParsableCommand {
    ...
    @Option(name: [.short, .customLong("timeout")], help: "Overall wait after sending probes (seconds)")
    ...
    private func runJSON(tracer: SwiftFTR) async throws {
        ...
        struct HopASN: Codable { let asn: Int; let prefix: String; let country_code: String; let name: String }
        ...
        for h in classified.hops {
            if let ip = h.ip {
                ...
                let asninfo = asnMap[ip].map { HopASN(asn: $0.asn, prefix: $0.prefix ?? "", country_code: $0.countryCode ?? "", name: $0.name) }
    ...
    private func runPretty(tracer: SwiftFTR) async throws {
        ...
        func catLabel(_ c: HopCategory) -> String {
            switch c { case .local: return "[LAN   ]"; case .isp: return "[ISP   ]"; case .transit: return "[TRANSIT]"; case .destination: return "[DESTINATION]"; case .unknown: return "[UNKNOWN]" }
        }
        ...
        for hop in classified.hops {
            if hop.ip == nil { print(String(format: "%2d", hop.ttl)); continue }
            let ip = hop.ip!
            ...
            let right: String
            if isPrivateIPv4(ip)      { right = "[Private Network]" }
            else if isCGNATIPv4(ip)   { right = "[CGNAT]" }
            else if let asn = hop.asn { right = "[AS\(asn) - \(hop.asName ?? "?")]" }
            else                      { right = "" }
        ...
        if let casn = classified.clientASN { print("Detected ISP: AS\(casn) (\(classified.clientASName ?? "?"))") }
        if let dasn = classified.destinationASN { print("Destination ASN: AS\(dasn) (\(classified.destinationASName ?? "?"))") }
    }
}
