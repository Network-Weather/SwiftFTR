...
func makeTimeExceededEmbedEcho(id: UInt16, seq: UInt16) -> [UInt8] {
  ...
  inner[3] = 0
  inner[4] = UInt8(id >> 8)
  inner[5] = UInt8(id & 0xFF)
  inner[6] = UInt8(seq >> 8)
  inner[7] = UInt8(seq & 0xFF)
  let ic = inetChecksum(inner)
  inner[2] = UInt8(ic >> 8)
  inner[3] = UInt8(ic & 0xFF)
  ...
  outer += ip
  outer += inner
  let oc = inetChecksum(outer)
  outer[2] = UInt8(oc >> 8)
  outer[3] = UInt8(oc & 0xFF)
  return outer
}
...
func testICMPParsing() throws {
  ...
  sin.sin_addr = in_addr(s_addr: UInt32(0x0102_0304).bigEndian)
...
func testClassification() async throws {
  ...
  try assert(
    cls.hops[2].category == HopCategory.isp && cls.hops[3].category == HopCategory.isp,
    "Gaps 3-4 sandwiched should be ISP")
  ...
  try assert(
    cls.hops[6].category == HopCategory.destination,
    "TTL7 same ASN as destination should be DESTINATION")
}
