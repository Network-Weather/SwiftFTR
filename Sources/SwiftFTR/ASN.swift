...
public struct ASNInfo: Sendable, Hashable, Codable {
    ...
    public init(asn: Int, name: String, prefix: String? = nil, countryCode: String? = nil, registry: String? = nil) {
        self.asn = asn; self.name = name; self.prefix = prefix; self.countryCode = countryCode; self.registry = registry
    }
}
...
public struct CymruWhoisResolver: ASNResolver {
        ...
        for line in text.split(separator: "\n") {
            ...
            if parts.count >= 7 {
                ...
                if let asn = Int(asStr) {
                    map[String(ipStr)] = ASNInfo(asn: asn, name: String(asName), prefix: prefix, countryCode: cc, registry: reg)
                }
            }
        }
        return map
    ...
    private func connectWhois(host: String, port: UInt16, timeout: TimeInterval) throws -> Int32 {
        ...
        guard getaddrinfo(host, String(port), &hints, &res) == 0, let info = res else { throw TracerouteError.resolutionFailed }
        ...
        var sa = sockaddr_in()
        memcpy(&sa, info.pointee.ai_addr, min(MemoryLayout<sockaddr_in>.size, Int(info.pointee.ai_addrlen)))
        ...
        if rv < 0 && errno == EINPROGRESS {
            ...
            if pr <= 0 { close(fd); throw TracerouteError.socketCreateFailed(errno: ETIMEDOUT) }
            // Check for error
            var err: Int32 = 0; var len = socklen_t(MemoryLayout<Int32>.size)
            if getsockopt(fd, SOL_SOCKET, SO_ERROR, &err, &len) != 0 || err != 0 { close(fd); throw TracerouteError.socketCreateFailed(errno: err) }
        } else if rv < 0 { close(fd); throw TracerouteError.socketCreateFailed(errno: errno) }
        ...
        for ip in ips {
            ...
            if let first = txts.first {
                ...
                if parts.count >= 4, let asField = parts.first {
                    ...
                    if let tok = tokens.first, let asn = Int(tok) ?? Int(tok.replacingOccurrences(of: "AS", with: "")) {
                        ...
                        result[ip] = ASNInfo(asn: asn, name: name ?? "", prefix: prefix, countryCode: cc, registry: reg)
                    }
                }
            }
        }
...
fileprivate func pickASName(from fields: [String]) -> String? {
    ...
    if fields.indices.contains(1) {
        ...
        let registries: Set<String> = ["arin","lacnic","ripe","ripencc","apnic","afrinic","jpnic","krnic"]
