...
func stunGetPublicIPv4(host: String = "stun.l.google.com", port: UInt16 = 19302, timeout: TimeInterval = 1.0) throws -> STUNPublicIP {
    // Resolve server
    ...
    guard getaddrinfo(host, String(port), &hints, &res) == 0, let info = res, let sa = info.pointee.ai_addr else { throw STUNError.resolveFailed }
        ...
        if atype == 0x0020 || atype == 0x0001 { // XOR-MAPPED-ADDRESS or MAPPED-ADDRESS
            if alen >= 8 {
                ...
                if family == 0x01 { // IPv4
                    ...
                    var addr: UInt32 = (UInt32(buf[ofs+4]) << 24) | (UInt32(buf[ofs+5]) << 16) | (UInt32(buf[ofs+6]) << 8) | UInt32(buf[ofs+7])
                    if atype == 0x0020 {
