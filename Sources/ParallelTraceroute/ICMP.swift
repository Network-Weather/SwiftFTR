...
func parseICMPv4Message(buffer: UnsafeRawBufferPointer, from saStorage: sockaddr_storage) -> ParsedICMP? {
    let addrStr: String = {
    ...
    case ICMPv4Type.timeExceeded.rawValue, ICMPv4Type.destinationUnreachable.rawValue:
        ...
        guard buffer.count - embedStart >= 28 else { // 20 (IP) + 8 (ICMP)
            return ParsedICMP(kind: (type == ICMPv4Type.timeExceeded.rawValue)
            ...
            if buffer.count - innerICMP >= 8 {
                ...
                if innerType == ICMPv4Type.echoRequest.rawValue {
                    ...
                    if type == ICMPv4Type.timeExceeded.rawValue {
                        return ParsedICMP(kind: .timeExceeded(originalID: id, originalSeq: seq), sourceAddress: addrStr)
                    } else {
                        return ParsedICMP(kind: .destinationUnreachable(originalID: id, originalSeq: seq), sourceAddress: addrStr)
                    }
                }
            }
        }
