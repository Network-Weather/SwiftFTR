...
final class SwiftFTRClassificationTests: XCTestCase {
    ...
    func testClassificationRulesAndHoleFilling() throws {
        ...
        let classified = try TraceClassifier().classify(trace: tr, destinationIP: destIP, resolver: resolver, timeout: 0.1)
    ...
    func testISPWhenClientASNMatchesHop() throws {
        ...
        let resolver = MockASNResolver(mapping: mapping)
        setenv("PTR_PUBLIC_IP", "198.51.100.50", 1)
        let classified = try TraceClassifier().classify(trace: tr, destinationIP: "203.0.113.200", resolver: resolver, timeout: 0.1)
    ...
    func testCGNATClassifiedAsISP() throws {
        ...
        let classified = try TraceClassifier().classify(trace: tr, destinationIP: "203.0.113.1", resolver: resolver, timeout: 0.1)
        XCTAssertEqual(classified.hops[0].category, .isp)
    ...
    func testDestinationCategoryWhenASMatchesDestination() throws {
        ...
        let resolver = MockASNResolver(mapping: mapping)
        let classified = try TraceClassifier().classify(trace: tr, destinationIP: "93.184.216.34", resolver: resolver, timeout: 0.1)
